...
If the answer requires opening Jupyter, the program isn't defensible. If the answer is one URL pointing at one case with one rule ID and one signal stack, the program is defensible — and the rest of the conversation becomes about *priorities*, not *trust*.
